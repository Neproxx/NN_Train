...
class Code_Completion_Context_CNN:
    ...
    def prepare_data(self, token_lists):
        ...
        if os.path.isfile(processed_training_data_dir + "data_y_train_context_" + str(context_length) + ".npz"):
            print("loading training data from .npy files...")
            xs_dict = numpy.load(processed_training_data_dir + "data_x_train_context_" + str(context_length) + ".npz")
            ys_dict = numpy.load(processed_training_data_dir + "data_y_train_context_" + str(context_length) + ".npz")
            xs = xs_dict[xs_dict.keys()[0]].tolist()
            ys = ys_dict[ys_dict.keys()[0]].tolist()
            print("load done")
        else:
            for token_list in token_lists:                          # token_list represents one single program
                for token_idx in range(len(token_list)):
                    # create training data for all possible hole_sizes
                    for extra_hole_size in range(max_hole_size):
                        # if enough tokens are left to the end of the list, create training instance with according hole_size
                        if token_idx + extra_hole_size + 1 < len(token_list):
                            ### create labels ###
                            token_strings_expected = token_list[token_idx:token_idx + extra_hole_size + 1]
                            #for i in range(token_idx + extra_hole_size):
                            #    token_strings_expected.append(token_list[i])

                            ### create input ###
                            prefix = token_list[:token_idx]
                            if token_idx + extra_hole_size + 1 < len(token_list):
                                suffix = token_list[token_idx + extra_hole_size + 1:]
                            else:
                                suffix = []

                            # get one_hot_encodings
                            prefix, suffix = self.trim_to_length(prefix, suffix, context_length)
                            expected = self.pad_prediction_data(token_strings_expected)

                            # create data for NN
                            xs.append(self.one_hot_tokenlist(prefix) + self.one_hot_tokenlist(suffix))
                            ys.append(self.one_hot_tokenlist(expected))

            # save data for later retraining
            print("saving training data to .npy files...")
            numpy.savez_compressed(processed_training_data_dir + "data_x_train_context_" + str(context_length) + ".npz", numpy.array(xs))
            numpy.savez_compressed(processed_training_data_dir + "data_y_train_context_" + str(context_length) + ".npz", numpy.array(ys))
            print("save done")

        return (xs, ys)

    def pad_prediction_data(self, tokens:list):
        """
        takes a list of tokens (as dicts) and returns a list of one hot encodings of the length of max_hole_size
        """
        if len(tokens) - max_hole_size != 0:
            needed_padding = len(tokens) - max_hole_size
            tokens = tokens + [{"type": "noToken", "value": "noToken"}] * needed_padding
    ...
    def one_hot_tokenlist(self, tokenlist):
        ...
        """for token_list in token_lists:       # OLD from prepare_data()
